...
interface Props {
  bets: Bet[]; // apostas do mês visível (ou todas — filtramos na mesma)
  year: number;
  month: number; // 0..11
...
const leadingBlanks = (y: number, m: number) => {
  const wd = new Date(y, m, 1).getDay(); // 0 = domingo
  return wd === 0 ? 6 : wd - 1; // semana começa à segunda
};
  ...
  return (
    <div className="card p-4 sm:p-4.5">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button onClick={next} className="btn-ghost h-8 w-8 !px-0" aria-label="Mês seguinte">
